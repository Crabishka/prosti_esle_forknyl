...
class PdfPageViewState extends State<PdfPageView> {
  ...
  Widget build(BuildContext context) {
    ...
    final double heightSpacing = widget.scrollDirection == PdfScrollDirection.horizontal ? 0.0 : pageSpacing;
    ...
    return AnimatedCrossFade(
        ...
        builder: (BuildContext context) {
          {
            ...
            final Widget image = widget.imageStream != null
                ? Image.memory(
                    widget.imageStream!,
                    width: widget.width,
            ...
            final Size originalPageSize = widget.pdfDocument!.pages[widget.pdfViewerController.pageNumber - 1].size;
            _heightPercentage = (isRotatedTo90or270 ? originalPageSize.width : originalPageSize.height) /
            ...
            final Widget canvas = (kIsDesktop && !widget.isMobileWebView && canvasRenderBox != null)
                ? RotatedBox(
                    ...
                    child: Listener(
                      ...
                      child: RawKeyboardListener(
                        ...
                        onKey: (RawKeyEvent event) {
                          final bool isPrimaryKeyPressed = kIsMacOS ? event.isMetaPressed : event.isControlPressed;
                          if ((canvasRenderBox!.getSelectionDetails().mouseSelectionEnabled ||
                          ...
                          if (event is RawKeyDownEvent) {
                            if (event.logicalKey == LogicalKeyboardKey.home ||
                              ...
                              widget.pdfViewerController.jumpToPage(widget.pdfViewerController.pageCount);
                          ...
                          onHover: (PointerHoverEvent details) {
                            setState(() {
                              if (canvasRenderBox != null) {
                                if (widget.interactionMode == PdfInteractionMode.selection) {
                                  final bool isText =
                                      canvasRenderBox!.findTextWhileHover(details.localPosition) != null;
                                  final bool isTOC = canvasRenderBox!.findTOC(details.localPosition);
                                  if (isTOC) {
      ...
      secondChild: Builder(
        builder: (context) {
          bool isVisible;
          if (widget.pageIndex >= widget.startPageIndex - 1 && widget.pageIndex <= widget.endPageIndex - 1) {
            isVisible = true;
          } else {
            isVisible = false;
          }
            ...
            child: Center(
              child: CircularProgressIndicator(
                valueColor: AlwaysStoppedAnimation<Color>(
                    _pdfViewerThemeData!.progressBarColor ?? (Theme.of(context).colorScheme.primary)),
      ...
      crossFadeState: widget.imageStream != null ? CrossFadeState.showFirst : CrossFadeState.showSecond,
